...
- 🔍 **历史搜索**：按文本、预览内容或文件名实时过滤；支持日期检索（`date:昨天 评审`、`date:今天 10:30`）
- 📅 **日期分组与时间显示**：列表按天分组（今天/昨天/日期），非今天的条目入库时间带日期
- ⬆️ **返回顶部**：长列表滚动后右下角浮出按钮，一键平滑回顶
...
| **搜索历史** | 点击搜索框后输入关键词；支持日期检索：`date:今天` / `date:昨天` / `date:09-02` / `date:2026-09-02` / `date:2026`，纯日期串（如 `09-02`）自动识别，可组合：`date:昨天 评审`（日期+文本）、`date:今天 10:30`（日期+时间定位） |
| **复制历史条目** | 点击条目中的复制操作 |
| **粘贴指定条目** | 双击历史条目 |
| **定位文件** | 单击文件条目，在 Explorer 中显示文件位置 |
| **返回顶部** | 历史列表滚动超过一屏后，点击右下角悬浮按钮回到顶部 |
